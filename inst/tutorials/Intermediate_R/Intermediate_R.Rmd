...
output$turnin <- renderText({
  stamp <- paste("Finished Chapter 1:", Sys.time(), input$name)
  paste0(stamp, "\n",
...
```{r, context="server"}
output$turnin <- renderText({
  stamp <- paste("Finished Chapter 2:", Sys.time(), input$name)
  paste0(stamp, "\n",
         digest::digest(stamp, "md5", serialize=FALSE))
})
```
...
```{r define-hello-ex-2, exercise=TRUE}
...
```{r, context="server"}
output$turnin <- renderText({
  stamp <- paste("Finished Chapter 3:", Sys.time(), input$name)
  paste0(stamp, "\n",
         digest::digest(stamp, "md5", serialize=FALSE))
})
```
