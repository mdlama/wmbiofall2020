...
question(
  ...
  answer("`a` and `b` equal 7 and 6 respectively after executing this code block."),
  answer("After the first call of `increment()`, where `a` is defined, `a` equals 7 and count equals 5."),
  answer("In the end, `count` will equal 10.", correct=TRUE),
  answer("In the last expression, the value of `count` was actually changed because of the explicit assignment.")
)
```
...
```{r toxo-ex-1, exercise=TRUE}

```

```{r toxo-ex-1-solution}
toxoData <- read.csv("https://whitlockschluter.zoology.ubc.ca/wp-content/data/chapter02/chap02q32ToxoplasmaAccidents.csv")
...
```{r mosaic-pipe-setup, echo=FALSE}
library(tidyverse)
```

```{r toxo-ex-2, exercise=TRUE, exercise.setup="mosaic-pipe-setup"}

```

```{r toxo-ex-2-solution}
read.csv("https://whitlockschluter.zoology.ubc.ca/wp-content/data/chapter02/chap02q32ToxoplasmaAccidents.csv") %>%
  table() %>%
  mosaicplot()
```

```{r toxo-ex-2-check}
gradethis::grade_code("This example illustrates another possible benefit of the pipe operator - avoiding intermediate variables. Note that sometimes the intermediate variables are useful, such as the contingency table `toxoTable` in this case.")
```
