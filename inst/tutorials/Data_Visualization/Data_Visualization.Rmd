...
The most common case for a bar plot is when we have one categorical variable and we want to count the number of occurrences of each category in the data. In order to do this, we create a contingency table consisting of the categories (or levels) of the variable and the counts of each level in the data. The following image illustrates this with the `site` variable.
...
gradethis::grade_code("Why doesn't `scale_x_discrete()` work? It is because the data type of the x aesthetic is double.")
```
...
```{r prop-subset-ex-1, exercise=TRUE, exercise.setup="stemdata-setup"}
ggplot(data = stemdata) +
  geom_bar(aes(x = site,
               y = after_stat(count/sum(count))),
           stat = "count")
...
*Modify the boxplot code to show a boxplot of apical height for each site. While you're at it, map site to fill as well.*
...
*Modify the boxplot code below to plot herbivory average* (`herb_avg`) *versus site.*
...
gradethis::grade_code("Remember: the log scale is performed first, and then the boxplot statistics are performed. So the quartiles that you see in the resulting plot are the quartiles of log herbivory.")
```
...
First, it is helpful to note that `geom_crossbar` uses the aesthetics `ymin`, `y`, and `ymax` to specify the crossbar. To complete the challenge, review the computed variables from the boxplot statistic and think about when and how to map original data and compute data to the aesthetics. By _when_, we mean either during the _start_ stage or _after_stat_ stage; by _how_ we mean which original/computed data gets mapped to which aesthetics at each stage. Use the hints to help walk you through it!
...
# We use the after_stat function to signify that we
...
```{r stacked-barplot-ex-3, exercise=TRUE, exercise.setup="stemdata-setup"}
ggplot(data = stemdata) +
  geom_bar(aes(x = site, 
               fill = year))
```
...
The following figure extends Figure 1 to include all of the steps in ggplot2's process, including the newly introduced position step. It implements the modified bar plot where we mapped `year` to `fill` and uses the default "stack" position adjustment.
...
```{r dodged-barplots-ex-1, exercise=TRUE, exercise.setup="stemdata-setup"}
ggplot(data = stemdata) +
  geom_bar(aes(x = site, 
               fill = year))
```
...
```{r fill-barplots-ex-1, exercise=TRUE, exercise.setup="stemdata-setup"}
ggplot(data = stemdata) +
  geom_bar(aes(x = site, 
               fill = year))
```
...
#### Exercise: Socially distance your geoms
...
```{r coord-flip-ex-1, exercise=TRUE, exercise.setup="stemdata-setup", exercise.eval=TRUE}
ggplot(data = stemdata) +
  geom_bar(aes(x = site))
```
...
Note how the resulting line is log-transformed to a curve!
...
There is quite a bit of overlapping geoms and points here, so instead of assigning `site` to the `group` variable, let's assign `site` to facets using `facet_wrap`:
...
To specify which variable we are facetting by, we use the `vars()` function, where the argument to the function is one of the original data **var**iables. The `facet_wrap` argument above can be thought of as assigning site to columns (the default), which has the effect of putting panels down for each level of site along columns first, then wrapping to the next row once it reaches the end of the plotting space.
...
The real gem here is the `summary` statistic that will compute summary statistics for your data. You can think of the `summary` statistic as a generalization of the `count` and `bin` statistics, as it can compute other aggregate statistics.

The easiest use of the `summary` statistic is to specify the aggregate function as a string with the `fun` argument. The function specified by the `fun` argument should accept a vector of values and return a number (i.e. an aggregate). The "summary" statistic requires both an `x` and `y` aesthetic and will compute the statistic given by `fun` on the `y` values subsetted on the levels of the `x` variable. Let's compute a bar plot where the heights of the bars represent the average apical height in June for each site (using the `mean` function).
...
Note the `width` argument to `geom_errorbar` which controls their width. If you change the order of the geoms, you will get a common plot called a "dynamite plot."
...
Nice work! You've completed your initial foray into ggplot2 and learned about the grammar of graphics. No one remembers all of the commands (there are a lot!), but hopefully the intuitive nature of the grammar allows you to think about how to construct all kinds of plots that we haven't covered. Use Figure 2 (shown again below) as a reference and reminder of the steps! Also make sure to use the resources mentioned below and in the header of this tutorial!
...
### Congratulations! You have finished "Data Visualization with ggplot2"!
...
output$turnin <- renderText({
  stamp <- paste("Finished Data Visualization with ggplot2:", Sys.time(), input$name)
  paste0(stamp, "\n",
