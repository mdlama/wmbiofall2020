...
Use later: Processing of data into statistical summaries to show properties of your data, including uncertainty quantification and statistical models.
...
Looks like a super small border size! The default is set to 0.1.
...
How can we create these? With `ggplot2`, all you have to do is combine two `geom_tile` geom specifications with a `+` sign.
...
```{r adding-points-ex-1, exercise=TRUE, exercise.setup="tidyverse-setup"}
ggplot(data = tiles) +
  geom_tile(mapping = aes(x = x, y = y),
            width = 1,
            height = 1,
...
*Starting with the data and geoms from the previous two exercises (rectangles and points), add line segments that connect:*
...
Specify aesthetic mappings in the `ggplot()` function is akin to _setting default aesthetic mappings for all of your geoms to follow_. You can override those defaults for any subsequent geom layer you specify.

## Scales, Statistics and Positions
...
```{r tick-marks-ex-1, exercise=TRUE, exercise.setup="tiles-setup"}
ggplot(data = tiles,
       mapping = aes(x = x, y = y)) +
  geom_tile(width = 1,
            height = 1,
...
```{r scale-labels-ex-1, exercise=TRUE, exercise.setup="tiles-setup"}
ggplot(data = tiles,
       mapping = aes(x = x, y = y)) +
  geom_tile(width = 1,
            height = 1,
...
```{r limits-ex-1, exercise=TRUE, exercise.setup="tiles-setup"}
ggplot(data = tiles,
       mapping = aes(x = x, y = y)) +
  geom_tile(width = 1,
            height = 1,
...
What, what?? What happened to our horizontal lines?! Setting the limits using the scale functions by default is different than just zooming in. By default, `ggplot2` will set the values of any data outside the limits to NA. Thus, the horizontal lines in this example will have one endpoint set to NA, and therefore _will not be drawn_. Thus, the default behavior in ggplot2 is to zoom **and** clip. 
...
```{r oob-ex-1, exercise=TRUE, exercise.setup="tiles-setup"}
ggplot(data = tiles,
       mapping = aes(x = x, y = y)) +
  geom_tile(width = 1,
            height = 1,
...
That's better! But wait - why is _clip_ (`oob_censor`) and not _zoom_ (`oob_keep()`) the default behavior? Clipping will make more sense when we discuss statistics later in this tutorial.

##### Location scales: Axis transformations

Besides setting axes labels, tick marks, and clipping/zooming windows, scales can also explicitly define the transformation from your data to the aesthetic (axis). By default, location scales do NOT perform a transformation, i.e. they perform an identity mapping between your data and the axes. There are times, however, when you will want to change this. We will cover one of the more common transformations - log transformations.

To illustrate transformations, let's revisit the milkweed data. The following creates a scatter plot of herbivory average versus apical stem height in June (cm). For simplicity, we first filter out where herbivory average is 0.
...
It appears that 0-0.3 was _stretched_, whereas 0.3-6 was _squished_, and that the rate of stretching and squishing varies across the axis (proportional to the derivative of the scaling function at each point).
...
You'll notice that (1) the label is now `log_herb_avg`, and (2) the minimum is around -1.5 in this plot, which corresponds to $10^{-1.5} \approx 0.03$ on the previous plot. Thus, the vertical axis is now showing the exponents, which while accurately depicting the log scale can be harder to interpret.
...
| :------ | :------- | :--------- | :-------- |
| [HCL](https://ggplot2.tidyverse.org/reference/scale_hue.html) (_default_)| `scale_*_hue` | - | [More info...](http://hclwizard.org/) |
...
By default, `ggplot2` maps to the [HCL color scale](http://hclwizard.org/#hcl-color-space) (H = Hue, C = Chroma, L = Luminance), which is a color scale that apparently maps well to human color perception.
...
```{r mapping-color-ex-1, exercise=TRUE, exercise.setup="tidyverse-setup"}
tiles <- tribble(
  ~x, ~y, ~fill,
  1, 1, "forestgreen",
  1, -1, "goldenrod1",
...
Huh? That's not right at all! What are those colors? Impostors!! 

Okay, well, they're not so bad - they are two equally distant (in hue) colors in HCL color space and represent ggplot2's default color palette. You can see these colors by using functions from the `scales` package as follows:
...
So how do we get the colors that are directly listed in the data? After all, our current `tiles` data is already representing aesthetic data. One way is to tell `ggplot2` that we want it to behave more like a location scale and use an identity mapping, which we can do by specifying the `scale_fill_identity()` function.
...
That was easy! Basically, `scale_fill_identity()` tells `ggplot2` that the data already specifies the desired aesthetic values. However, most of the time our categorical data doesn't store actual color names. For example, suppose our data looked more like this:
...
```{r scale-manual-ex-1, exercise=TRUE, exercise.setup="tidyverse-setup"}
tiles <- tribble(
  ~x, ~y, ~royalty,
  1, 1, "william",
  1, -1, "mary",
...
##### Color scales: Set legend titles, keys, and key labels
...
##### Color scales: Adjust key limits
...
```{r}
ggplot(stemdata) + 
  geom_point(aes(x = h_apical, y = h_apical.next, color = herb_avg), alpha = 0.5) + 
...
ggplot(stemdata) + 
  geom_point(aes(x = h_apical, y = h_apical.next, color = herb_avg), alpha = 0.5) + 
  scale_color_viridis_c(trans = "log10", na.value = "red") + 
  scale_y_continuous(limits = c(0, 200))
```
...
ggplot(stemdata) + 
  geom_point(aes(x = h_apical, y = h_apical.next, color = herb_avg), alpha = 0.5) + 
  scale_color_viridis_c(trans = "log10", na.value = NA) + 
  scale_y_continuous(limits = c(0, 200))
```
...
ggplot(stemdata) + 
  geom_point(aes(x = h_apical, y = h_apical.next, color = herb_avg), alpha = 0.5) + 
  scale_color_viridis_c(trans = "log10", na.value = NA, direction = -1) + 
  scale_y_continuous(limits = c(0, 200))
```
...
ggplot(stemdata) + 
  ...
  scale_color_gradient2(low = "forestgreen", 
                        mid = "white", 
                        high = "goldenrod1", 
                        midpoint = mean(stemdata$herb_avg, na.rm = TRUE),
                        na.value = "forestgreen") + 
  scale_y_continuous(limits = c(0, 200)) +
  theme_dark()
```
...
What about tick labels? Axes labels? We'll learn about setting those later. Before we go there, let's look at our first complex geom `geom_bar` that automates a lot of this process. Here's the command in `ggplot2` using the `geom_bar` geom that performs this data processing and aesthetic mappings free of charge:
```{r}
...
The `geom_bar` geom performs a lot of things in this example:
 ...
 1. Use the statistic `stat_count()` to compute the contingency table of the number of plants in each site.
 2. Create default values for the `xmin`, `xmax`, `ymin`, and `ymax` based on parameters for the bars and the counts from the contingency table.
 3. Plot the results using `geom_rect` geoms and assign tick labels and axes labels automatically based on the raw and calculated data from the statistic.

```{r, echo=FALSE, out.width="95%", out.extra="class='center'", fig.cap="Figure 1. (1) Count all plants within each site using the `count` statistic, (2) compute aesthetics for the bars (xmin, xmax, ymin, ymax), and (3) map those aesthetics to the coordinate plane (in this case using `geom_rect` primitive)."}
...
stemdata %>%
  ggplot() +
  geom_bar(aes(x = site, fill = factor(year)))
```
...
stemdata %>%
  ggplot() +
  geom_bar(aes(x = site, fill = factor(year))) +
  geom_text(aes(x = site, label = year, group = year),
            stat = "count",
...
stemdata %>%
  ggplot() +
  geom_point(aes(x = site, y = herb_avg),
